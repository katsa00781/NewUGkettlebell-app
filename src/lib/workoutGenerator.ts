...
import { FmsAssessment as FMSAssessment } from './fms';
...
function generateDay1Plan(
  ...
  sections.push({
    ...
    exercises: [
      ...
      // Rotációs vagy rehab
      {
        exerciseId: rotaciosVagyRehab?.id || '',
...
function generateDay4Plan(
  ...
  sections.push({
    ...
    exercises: [
      {
        exerciseId: getRandomExercise(categorizedExercises, 'gait')?.id || '',
      ...
      {
        exerciseId: getRandomExercise(categorizedExercises, 'rehab')?.id || '',
    ...
    if (includeWeights) {
      sections.forEach(section => {
        section.exercises.forEach(exercise => {
          ...
          if (exercise.exerciseId && !exercise.weight) {
